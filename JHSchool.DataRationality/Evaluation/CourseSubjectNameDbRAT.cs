    ...
    public class CourseSubjectNameDbRAT : IDataRationality
    {
        ...
        public string Description
        {
            get
            {
                ...
                strBuilder.AppendLine("檢查範圍：學生狀態(一般、輟學)，各學年度學期俢課資料。");
                strBuilder.AppendLine("檢查項目：學生修習課程在單一學年度學期科目名稱有重覆");
                strBuilder.AppendLine("檢查意義：當學生單一學年度學期修習課程科目名稱有重覆時，在計算學期科目成績只會取其中一個課程成績計算，將造成科目成績有誤，此檢查功能可找出有疑問學生俢課課程，並可匯出資料或是將課程加入課程待處理。");
        ...
        public DataRationalityMessage Execute()
        {
            ...
            try
            {
                ...
                string strQuery_Err = "select student.id as sid,student.student_number,student.name,course.school_year,course.semester,course.subject,count(sc_attend.id) as ssid from student inner join sc_attend on student.id = sc_attend.ref_student_id inner join course on course.id=sc_attend.ref_course_id  where student.status in(1,8)  and course.subject <>'' group by sid,student.student_number,student.name,school_year,semester,subject having count(sc_attend.id)>1";
                ...
                if (Err_StudentIDList.Count > 0)
                {
                    ...
                    // -- 欄位：學生系統編號、學號、年級、班級、座號、學生姓名、學年度、學期、課程系統編號、課程名稱、科目名稱
                    string strQueryData = "select student.id as sid,student.student_number,class.grade_year,class.class_name,student.seat_no,student.name,course.school_year,course.semester,course.id as coid,course.course_name,course.subject from student inner join sc_attend on student.id = sc_attend.ref_student_id inner join course on course.id=sc_attend.ref_course_id left join class on student.ref_class_id=class.id where course.subject <>'' and student.id in(" + string.Join(",", Err_StudentIDList.ToArray()) + ")";
                    ...
                    foreach (DataRow drd in dt_Data.Rows)
                    {
                        foreach (DataRow dre in dt_Err.Rows)
                        {
                            ...
                            if (dre["sid"].ToString() == drd["sid"].ToString() && dre["school_year"].ToString() == drd["school_year"].ToString() && dre["semester"].ToString() == drd["semester"].ToString() && dre["subject"].ToString() == drd["subject"].ToString())
                            {
